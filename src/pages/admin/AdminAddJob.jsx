...
const INP = "w-full bg-slate-900 border border-slate-700 hover:border-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl px-4 py-2.5 text-sm text-white outline-none placeholder-slate-500 transition-all";
...
export default function AdminAddJob() {
  ...
  if (fetching) return <div className="flex items-center justify-center h-64"><Loader className="w-8 h-8 text-blue-400 animate-spin"/></div>;

  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={()=>navigate("/admin/jobs")} className="p-2 rounded-xl border border-slate-700 text-slate-400 hover:text-white hover:border-slate-500 transition-colors"><ChevronLeft className="w-4 h-4"/></button>
        <div>
          <h1 className="text-2xl font-bold text-white">{isEdit?"Edit Job":"Post New Job"}</h1>
          <p className="text-slate-400 text-sm mt-0.5">{isEdit?"Update the job posting details":"Fill in the details to create a new job posting"}</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Company Info */}
        <div className="bg-[#0d1a2d] border border-slate-800 rounded-2xl p-6 space-y-4">
          <h2 className="text-white font-semibold text-sm uppercase tracking-wide border-b border-slate-800 pb-3">Company Information</h2>
          <div className="flex items-start gap-6">
            <div className="shrink-0">
              <div onClick={()=>logoRef.current?.click()} className="w-20 h-20 rounded-2xl border-2 border-dashed border-slate-700 hover:border-blue-500 cursor-pointer flex items-center justify-center overflow-hidden transition-colors bg-slate-900 group">
                {logoPreview ? <img src={logoPreview} alt="logo" className="w-full h-full object-cover"/> : <Upload className="w-6 h-6 text-slate-600 group-hover:text-blue-400"/>}
              </div>
              <input ref={logoRef} type="file" accept="image/*" onChange={handleLogo} className="hidden"/>
              <p className="text-xs text-slate-600 mt-1 text-center">Company Logo</p>
            </div>
        ...
        {/* Job Details */}
        <div className="bg-[#0d1a2d] border border-slate-800 rounded-2xl p-6 space-y-4">
          <h2 className="text-white font-semibold text-sm uppercase tracking-wide border-b border-slate-800 pb-3">Job Details</h2>
        ...
        {/* Skills */}
        <div className="bg-[#0d1a2d] border border-slate-800 rounded-2xl p-6 space-y-4">
          <h2 className="text-white font-semibold text-sm uppercase tracking-wide border-b border-slate-800 pb-3">Skills & Requirements</h2>
          <div>
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button type="button" onClick={()=>addSkill("skills",skillInput,setSkillInput)} className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-xl transition-colors"><Plus className="w-4 h-4"/></button>
            </div>
            <div className="flex flex-wrap gap-2">
              {form.skills.map((s,i)=><span key={i} className="inline-flex items-center gap-1.5 bg-blue-500/15 border border-blue-500/30 text-blue-400 text-xs px-3 py-1 rounded-full"><span>{s}</span><button type="button" onClick={()=>removeSkill("skills",i)}><X className="w-3 h-3"/></button></span>)}
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button type="button" onClick={()=>addSkill("preferredSkills",prefInput,setPrefInput)} className="px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium rounded-xl transition-colors"><Plus className="w-4 h-4"/></button>
            </div>
            <div className="flex flex-wrap gap-2">
              {(form.preferredSkills||[]).map((s,i)=><span key={i} className="inline-flex items-center gap-1.5 bg-slate-700 border border-slate-600 text-slate-300 text-xs px-3 py-1 rounded-full"><span>{s}</span><button type="button" onClick={()=>removeSkill("preferredSkills",i)}><X className="w-3 h-3"/></button></span>)}
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#0d1a2d] border border-slate-800 rounded-2xl p-6 space-y-4">
          <h2 className="text-white font-semibold text-sm uppercase tracking-wide border-b border-slate-800 pb-3">Eligibility Criteria <span className="text-slate-600 font-normal normal-case">(optional)</span></h2>
          <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex gap-3 justify-end">
          <button type="button" onClick={()=>navigate("/admin/jobs")} className="px-6 py-2.5 border border-slate-700 text-slate-300 hover:border-slate-500 hover:text-white rounded-xl text-sm font-medium transition-colors">Cancel</button>
          <button type="submit" disabled={loading} className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white text-sm font-semibold rounded-xl transition-colors">
